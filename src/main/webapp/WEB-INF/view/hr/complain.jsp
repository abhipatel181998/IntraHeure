							...
							<table class="table table-striped custom-table m-b-0"
								...
								<tbody>
									<c:forEach items="${complainData }" var="data">
										<c:if
											test="${data.complainTo.loginId eq sessionScope.loginId }">
											<c:forEach items="${employeeData }" var="employee">
												<c:if
													test="${data.complainFrom.loginId  eq employee.loginVo.loginId}">
													<tr>
														<td>${data.complainId }</td>
														<td><a
															href="viewHrProfile?userId=${employee.userId }"
															class="avatar"><c:if
																	test="${employee.profilePic eq null}">
														${fn:substring(employee.firstName,0,1)}
													</c:if> <c:if test="${employee.profilePic ne null}">
																	<img class="avatar"
																		src="${pageContext.request.contextPath }/profilePics/${employee.profilePic}"
																		alt="user">
																</c:if></a>
															<h2>
																<a href="viewEmployeeProfile?userId=${employee.userId }">${employee.firstName }
																	${employee.lastName }<span>${employee.departmentVo.departmentName }(${employee.designationVo.designationName })</span>
																</a>
															</h2></td>
